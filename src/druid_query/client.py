...
class Client:
    ...
    def execute(self, query: Query):
        if isinstance(query, NativeQuery):
            ...
            resp = self.inner.post(self.native_endpoint, data=gzip.compress(
                json.dumps(query, default=druid_serealize).encode('utf-8')))
        elif isinstance(query, Sql):
            assert self.sql_endpoint is not None
            resp = self.inner.post(self.sql_endpoint, data=gzip.compress(
                json.dumps(query, default=druid_serealize).encode('utf-8')))
        else:
        ...
        if resp.status_code in [400, 429, 501, 504, 500]:
            ...
            if isinstance(query, NativeQuery):
                return resp.json()
            elif query.result_format in ['object', 'array']:
                return resp.json()
